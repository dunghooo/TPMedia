...
function MemberCard() {
  ...
  const memberImages = [image2, acc1, acc2];
  return (
    <div className="member-card">
      ...
      <div className="member-card-body">
        ...
        <div className="Member-image">
          {" "}
          {/* Ảnh lớn */}{" "}
          <img className="Member-main-image" src={selectedImage} alt="Member" />{" "}
          {/* 3 ảnh nhỏ */}{" "}
          <div className="Member-image-slider">
            {" "}
            {memberImages.map((image, index) => (
              <div
                key={index}
                className={`Member-thumbnail ${selectedImage === image ? "active" : ""}`}
                onClick={() => setSelectedImage(image)}
              >
                {" "}
                <img src={image} alt={`Member ${index + 1}`} />{" "}
              </div>
            ))}{" "}
          </div>{" "}
        </div>{" "}
